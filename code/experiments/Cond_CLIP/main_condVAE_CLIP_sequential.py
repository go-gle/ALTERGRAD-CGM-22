...
parser.add_argument('--dim-condition', type=int, default=32, help="Dimensionality of conditioning vectors for conditional generation (default: 128)")
...
if args.train_autoencoder:
    ...
    for epoch in range(1, args.epochs_clip+1):
        ...
        for data in train_loader_clip:
            data = data.to(device)
            optimizer.zero_grad()
            #loss, clip_loss, kld = autoencoder.compute_clip_loss(data)
            loss = autoencoder.compute_clip_loss(data)
            #train_loss_all_kld += kld.item()
            #train_loss_all_clip += clip_loss.item()
            cnt_train+=1
            loss.backward()
        ...
        for data in val_loader_clip:
            ...
            #val_loss_all_clip += clip_loss.item()
        ...
        if epoch % 1 == 0:
            ...
            #print('{} Epoch: {:04d}, Train Loss: {:.3f}, Train CLIP Loss: {:.3f}, Train KLD: {:.3f}, Val Loss: {:.3f}, Val CLIP Loss: {:.3f}, Val KLD: {:.3f}'.format(dt_t, epoch, train_loss_all/cnt_train, train_loss_all_clip/cnt_train, train_loss_all_kld/cnt_train, val_loss_all/cnt_val, val_loss_all_clip/cnt_val, val_loss_all_kld/cnt_val))
        ...
        if best_val_loss >= val_loss_all:
            ...
            }, 'autoencoder_seqCLIP_cond.pth.tar')
            early_stopping_counts = 0
    ...
    checkpoint = torch.load('autoencoder_seqCLIP_cond.pth.tar')
...
autoencoder.freeze_graph_encoder()
autoencoder.freeze_feature_encoder()
#### JUST THE DECODER
...
if args.train_autoencoder:
    ...
    for epoch in range(1, args.epochs_autoencoder+1):
        ...
        for data in train_loader:
            data = data.to(device)
            optimizer.zero_grad()
            loss  = autoencoder.recon_loss(data)
        ...
        if epoch % 1 == 0:
            dt_t = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
            print('{} Epoch: {:04d}, Train Reconstruction Loss: {:.4f}, Val Reconstruction Loss: {:.4f}'.format(dt_t,epoch, train_loss_all/cnt_train, val_loss_all/cnt_val))
        ...
        for epoch in range(1, args.epochs_denoise+1):
            ...
            if best_val_loss >= val_loss_all:
                ...
                torch.save({
                    ...
                    'optimizer' : optimizer.state_dict(),
                }, 'denoise_model_seqCLIP_cond.pth.tar')
                early_stopping_counts = 0
            if early_stopping_counts == args.early_stopping_rounds:
                print('early stopping')
                break
    else:
        checkpoint = torch.load('denoise_model_seqCLIP_cond.pth.tar')
...
torch.save({
                'state_dict': autoencoder.state_dict(),
                'optimizer' : optimizer.state_dict(),
            }, f"runs/autoencoder_seqCLIP_cond_{str(mae)[:6].replace('.', '-')}.pth.tar")

# Save to a CSV file
with open(f"runs/seqCLIP_cond_{str(mae)[:6].replace('.', '-')}.csv", "w", newline="") as csvfile:
