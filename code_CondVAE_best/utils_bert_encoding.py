...
def preprocess_dataset_bert(dataset, n_max_nodes, spectral_emb_dim, n_cond):
    ...
    projection_matrix = torch.randn(input_dim, output_dim) / torch.norm(torch.randn(input_dim, output_dim), dim=0, keepdim=True)
        ...
        if dataset == 'test':
            ...
            if os.path.isfile(filename):
                ...
                for line in fr:
                    ...
                    feats_stats1 = outputs.last_hidden_state

                    # Apply attention mask-based pooling
                    attention_mask = inputs['attention_mask']
                    input_mask_expanded = attention_mask.unsqueeze(-1).expand(feats_stats1.size()).float()
                    sum_embeddings = torch.sum(feats_stats1 * input_mask_expanded, 1)
                    sum_mask = input_mask_expanded.sum(1)

                    # Mean pooling
                    pooled_embeddings = sum_embeddings / torch.clamp(sum_mask, min=1e-9)

                    # Project to desired dimensionality
                    feats_stats1 = torch.matmul(pooled_embeddings, projection_matrix)
                    #print(feats_stats1.shape)

                    feats_stats2 = extract_numbers(desc)
                    feats_stats2 = torch.FloatTensor(feats_stats2).unsqueeze(0).to(device)

                    feats_stats = torch.cat((feats_stats1, feats_stats2), axis=1)
                    ####
            ...
            if os.path.isfile(filename):
                ...
                for fileread in tqdm(files):
                    ...
                    with open(fstats, 'r') as f:
                        ...
                        feats_stats2 = extract_feats(fstats)
                        feats_stats2 = torch.FloatTensor(feats_stats2).unsqueeze(0).to(device)
                        feats_stats = torch.cat((feats_stats1, feats_stats2), axis=1)
                        ####
