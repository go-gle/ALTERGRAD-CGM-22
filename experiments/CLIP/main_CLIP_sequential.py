...
if args.train_autoencoder:
    ...
    checkpoint = torch.load('autoencoder_seqCLIP.pth.tar')
        ...
        for epoch in range(1, args.epochs_denoise+1):
            ...
            if best_val_loss >= val_loss_all:
                ...
                torch.save({
                    ...
                    'optimizer' : optimizer.state_dict(),
                }, 'denoise_model_seqCLIP.pth.tar')
                early_stopping_counts = 0
            if early_stopping_counts == args.early_stopping_rounds:
                print('early stopping')
                break
    else:
        checkpoint = torch.load('denoise_model_seqCLIP.pth.tar')
...
torch.save({
                'state_dict': autoencoder.state_dict(),
                'optimizer' : optimizer.state_dict(),
            }, f"runs/autoencoder_seqCLIP_{str(mae)[:6].replace('.', '-')}.pth.tar")

# Save to a CSV file
with open(f"runs/seqCLIP_{str(mae)[:6].replace('.', '-')}.csv", "w", newline="") as csvfile:
