...
autoencoder = CondVariationalAutoEncoder(args.spectral_emb_dim+1, args.hidden_dim_encoder, args.hidden_dim_decoder, args.latent_dim, args.n_layers_encoder, args.n_layers_decoder, n_max_nodes=args.n_max_nodes).to(device)
optimizer = torch.optim.Adam(autoencoder.parameters(), lr=args.lr)
scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=500, gamma=0.1)
        ...
        for epoch in range(1, args.epochs_denoise+1):
            ...
            if best_val_loss >= val_loss_all:
                ...
                torch.save({
                    ...
                    'optimizer' : optimizer.state_dict(),
                }, 'denoise_model_condVAE.pth.tar')
    else:
        checkpoint = torch.load('denoise_model_condVAE.pth.tar')
...
torch.save({
                'state_dict': autoencoder.state_dict(),
                'optimizer' : optimizer.state_dict(),
            }, f"runs/autoencoder_condVAE_{str(mae)[:6].replace('.', '-')}.pth.tar")

# Save to a CSV file
with open(f"runs/condVAE_{str(mae)[:6].replace('.', '-')}.csv", "w", newline="") as csvfile:
